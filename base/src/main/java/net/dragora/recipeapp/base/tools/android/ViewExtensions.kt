...
import net.dragora.recipeapp.base.tools.android.Consts.HOUR_IN_MILLIS
import net.dragora.recipeapp.base.tools.android.Consts.MINUTE_IN_MILLIS
import net.dragora.recipeapp.base.tools.android.Consts.SECOND_IN_MILLIS
...
fun Long.formatDuration(res: StringRetriever): CharSequence {
    ...
    return when {
        this >= HOUR_IN_MILLIS -> {
            ...
            res.getQuantityString(R.plurals.common_hours, hours, hours)
        }
        this >= MINUTE_IN_MILLIS -> {
            val minutes = ((this + 30000) / MINUTE_IN_MILLIS).toInt()
            res.getQuantityString(R.plurals.common_minutes, minutes, minutes)
        }
        else -> {
            val seconds = ((this + 500) / SECOND_IN_MILLIS).toInt()
            res.getQuantityString(R.plurals.common_seconds, seconds, seconds)
        }
    }
}
